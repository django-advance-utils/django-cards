...
ask for a header only when there is something to put in it; ``default``, ``accordion``, ``table``,
``datatable`` and ``linked_datatables`` draw one whenever ``show_header`` is set. Both must honour
``show_header`` being off. (``message`` is the third shape -- a title is the only thing that earns
it a header -- and has its own module, ``test_show_header_message_table``, which also builds the
``table`` and ``datatable`` cards for real.)
...
SWITCH_ONLY = ['default', 'accordion', 'table', 'datatable', 'linked_datatables']
