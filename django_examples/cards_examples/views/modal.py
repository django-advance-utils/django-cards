...
class ModalExampleIndex(MainMenu, CardMixin, TemplateView):
    ...
    def form_modals(self):
        self.add_menu(menu_name='form_modals', menu_type='buttons').add_items(
            MenuItem(url='cards_examples:form_modal', menu_display='Form Modal'))


class SimpleModal(CardMixin, Modal):
...
class CardFormMixin(CardMixin, FormModal):
    ...
    def add_form_card(self, form):
        card = self.add_card(template_name='modal',
                             ...
                             form=form)
        self.add_form_card_field(card)
        return card

    def add_form_card_field(self, card):
        pass


class ExampleFormModal(CardFormMixin):
